...
System deps (only if using the bundled adapters): `claude` CLI on PATH (coder), `codex` CLI on PATH (only for `codexCoder`), `ffmpeg` on PATH (renderer), Chromium installed once via `npx playwright install chromium` (the `playwright` package no longer downloads browsers on install).
