...
System deps (only if using the bundled adapters): `claude` CLI on PATH (coder), `codex` CLI on PATH (only for `codexCoder`), `ffmpeg` on PATH (renderer), Chromium auto-downloaded by `playwright` postinstall.
