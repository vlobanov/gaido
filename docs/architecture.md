...
System deps (only if using the bundled adapters): `claude` CLI on PATH (coder), `codex` CLI on PATH (only for `codexCoder`), `opencode` CLI on PATH (only for `opencodeCoder`), `cursor-agent` CLI on PATH (only for `cursorCoder`), `ffmpeg` on PATH (renderer), Chromium installed once via `npx playwright install chromium` (the `playwright` package no longer downloads browsers on install).
